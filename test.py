...
class TestBuildKernel(unittest.TestCase):
    ...
    def test_version_compare_major(self):
        old = build_kernel.version_cmp(build_kernel.VersionInfo(version_triple="4.7.10", vmlinuz="vmlinuz-4.7.10-gentoo",
                                           system_map="System.map-4.7.10-gentoo", config="config-4.7.10-gentoo", is_old=False))
        new = build_kernel.version_cmp(build_kernel.VersionInfo(version_triple="5.7.10", vmlinuz="vmlinuz-5.7.10-gentoo",
                                           system_map="System.map-5.7.10-gentoo", config="config-5.7.10-gentoo", is_old=False))
        # Greater is essentially newer
        self.assertGreater(new, old)

    def test_version_compare_minor(self):
        old = build_kernel.version_cmp(build_kernel.VersionInfo(version_triple="5.6.10", vmlinuz="vmlinuz-5.6.10-gentoo",
    ...
    def test_version_compare_patch(self):
        old = build_kernel.version_cmp(build_kernel.VersionInfo(version_triple="5.7.9", vmlinuz="vmlinuz-5.7.9-gentoo",
    ...
    def test_version_compare_old(self):
        old = build_kernel.version_cmp(build_kernel.VersionInfo(version_triple="5.7.10", vmlinuz="vmlinuz-5.7.10-gentoo.old",
