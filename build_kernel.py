...
def error_and_exit(error):
    ...
    print(Fore.RED + f"{sys.argv[0]}: " + Style.RESET_ALL + f"Error! {error}, exiting...")
    sys.exit(1)
...
class KernelUpdater:
    ...
    def __update_config(self):
        ...
        src = INSTALL_PATH / self.current_kernels[0].config # The newest config we have
    ...
    def __compile_kernel(self):
        ...
        try:
            subprocess.run(["/bin/bash", "-c", "\"make\" --jobs $(nproc) --load-average $(nproc)"], check=True)
        except CalledProcessError as err:
            error_and_exit(err)
    ...
    def __install_new_kernel(self):
        ...
        script_info(f"Installing kernel image, system map, and config to {str(INSTALL_PATH)}")
        try:
    ...
    def __find_installed_kernels(self):
        ...
        for vmlinuz in vmlinuzes:
            ...
            self.current_kernels.append(VersionInfo(version_triple, vmlinuz, system_map, config, is_old))
    ...
    def __clean_up(self):
        ...
        if len(self.current_kernels) <= MAX_VERSIONS_TO_KEEP:
            script_info(f"Only {len(self.current_kernels)} kernels are there, not deleting any")
            return
        ...
        for _ in range(num_to_delete):
            ...
            if kernel_to_delete.is_old:
                script_info(f"Deleting version {kernel_to_delete.version_triple}.old")
            else:
                script_info(f"Deleting version {kernel_to_delete.version_triple}")

            # Evidently "unlink" is the same as remove
            script_info(f"Deleting file {str(kernel_to_delete.vmlinuz.absolute())}")
            kernel_to_delete.vmlinuz.unlink()
            script_info(f"Deleting file {str(kernel_to_delete.system_map.absolute())}")
            kernel_to_delete.system_map.unlink()
            script_info(f"Deleting file {str(kernel_to_delete.config.absolute())}")
